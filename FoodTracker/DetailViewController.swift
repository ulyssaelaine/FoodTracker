...
class DetailViewController: UIViewController {
    ...
    func requestAuthorizationForHealthStore(){
        ...
        store.healthStore?.reuestAuthorizationToShareTypes(NSSet(array: dataTypesToWrite), readTypes: NSSet(array: dataTypesToRead), completion: {
    ...
    func saveFoodItem (foodItem : USDAItem){
        ...
        if HKHealthStore.isHealthDataAvailable() {
            ...
            let energyUnit = HKQuantity(unit: HKUnit.kiloclorieUnit(), doubleValue: (foodItem.energy as NSString).doubleValue)
            ...
            let foodCoorelation = HKCorrelation(type: HKCorrelationType.correlationTypeForIdentifier(HKCorrelationTypeIdentifierfood), startDate: timeFoodWasEntered, endDate: timeFoodWasEntered, objects: foodDataSet, metadata: foodMetaData, metadata: foodMetaData)
            
            var store: HealthStoreConstant = HealthStoreConstant()
            store.healthStore?.saveObject(foodCoorelation, withCompletion: { (sucess, error) -> Void in
                if success {
